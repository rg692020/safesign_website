<?php
	$sprache=Array();

	$sprache[0]['Name']='Deutsch';
	$sprache[1]['Name']='english';
	$sprache[2]['Name']='français';
	$sprache[3]['Name']='español';

    //Start of navbar translation
    $sprache[0]['Produkt']='Unser Produkt';
    $sprache[1]['Produkt']='Our product';
    $sprache[2]['Produkt']='Notre produit';
    $sprache[3]['Produkt']='Nuestro producto';

    $sprache[0]['Kontakt']='Kontakt';
    $sprache[1]['Kontakt']='Contact';
    $sprache[2]['Kontakt']='Contact';
    $sprache[3]['Kontakt']='Contacto';

    $sprache[0]['Impressum']='Impressum';
    $sprache[1]['Impressum']='imprint';
    $sprache[2]['Impressum']='empreinte';
    $sprache[3]['Impressum']='pie de imprenta';

    $sprache[0]['Ueberschrift']='Hauptseite';
    $sprache[1]['Ueberschrift']='Homepage';
    $sprache[2]['Ueberschrift']='sité du main';
    $sprache[3]['Ueberschrift']='página principal';

?>
